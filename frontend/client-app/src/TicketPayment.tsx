...
const TicketPayment: React.FC<TicketPaymentProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setDepartureDate(defaultStart.toISOString().split('T')[0]);
      setArrivalDate(defaultEnd.toISOString().split('T')[0]);
    }
  }, [isOpen]);
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ color: '#B76E3C', fontWeight: 'bold', fontSize: '18px' }}>
                  {formatPrice(totalPrice || tour?.price || 0)}
                </div>
              </div>
              {tour && (
                <>
                  <div>
                    <div style={{ color: '#8B5A2B', fontSize: '12px', marginBottom: '3px' }}>Маршрут</div>
